...
public class EncounterController {
    ...
    public ApiResponse<EncounterResponse> create(@Valid @RequestBody CreateEncounterRequest request) {
        return ApiResponse.<EncounterResponse>builder().success(true).message("Encounter created").data(service.create(request)).build();
    }

    @GetMapping("/{encounterId}")
    @Operation(summary = "Get encounter by ID")
    public ApiResponse<EncounterResponse> getById(@PathVariable UUID encounterId) {
        return ApiResponse.<EncounterResponse>builder().success(true).data(service.getById(encounterId)).build();
    }
    ...
    public ApiResponse<List<EncounterResponse>> getByPatientFile(@PathVariable UUID patientFileId) {
        return ApiResponse.<List<EncounterResponse>>builder().success(true).data(service.getByPatientFile(patientFileId)).build();
    }

    @PutMapping("/{encounterId}")
    @Operation(summary = "Update encounter notes, complaint, or status")
    public ApiResponse<EncounterResponse> update(@PathVariable UUID encounterId, @Valid @RequestBody UpdateEncounterRequest request) {
        return ApiResponse.<EncounterResponse>builder().success(true).message("Encounter updated").data(service.update(encounterId, request)).build();
    }
    ...
    public ApiResponse<EncounterResponse> complete(@PathVariable UUID encounterId) {
        return ApiResponse.<EncounterResponse>builder().success(true).message("Encounter completed").data(service.complete(encounterId)).build();
    }
    ...
    public ApiResponse<EncounterResponse> cancel(@PathVariable UUID encounterId) {
        return ApiResponse.<EncounterResponse>builder().success(true).message("Encounter cancelled").data(service.cancel(encounterId)).build();
    }
